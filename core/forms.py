...
from core.models import User, Asset, AssetAllocation, ResourceBooking, MaintenanceRequest, IoTDevice
...
class CustomUserCreationForm(UserCreationForm):
    ...
    def clean(self):
        ...
        if role and role != User.EMPLOYEE:
            raise forms.ValidationError("You are not authorized to request an elevated role.")
        return cleaned_data
...
class IoTDeviceForm(forms.ModelForm):
    class Meta:
        model = IoTDevice
        fields = ['asset', 'device_id', 'status', 'alert_thresholds']
